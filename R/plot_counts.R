...
#'        visualisation (deprecated if log.scale is TRUE)
#' @param log.scale Logical. If TRUE (default), uses a log10 scale for the y-axis,
#'        which handles dominant species visualization automatically.
...
#' }
#'
plot_species_counts <- function(df, confidence = 0, remove.dominants = FALSE, log.scale = TRUE) {
  ...
  if (log.scale) {
    # If log scale is on, we generally don't need to remove dominants as the scale handles it.
    # We plot everything on log10 scale.
    plot <- ggplot2::ggplot(
      df1,
      ggplot2::aes(
    ...
    # OLD LOGIC for Linear Scale with optional Dominant Removal
