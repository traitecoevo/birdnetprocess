test_that("quickcalls returns a ggplot object", {
    # Mock data
    df <- dplyr::tibble(
    ...
    p <- quickcalls(df, confidence = 0.5, remove.dominants = FALSE)
...
test_that("quickcalls handles remove.dominants = TRUE", {
    ...
    # The logic in quickcalls seems hard to trigger with small numbers or specific distributions.
    ...
    expect_output(
        p <- quickcalls(df, confidence = 0.5, remove.dominants = TRUE),
...
test_that("quickcalls errors if not enough species", {
    df <- dplyr::tibble(
        `Common Name` = c("Robin"),
        Confidence = c(0.9),
        start_time = lubridate::ymd_hms("2024-01-01 10:00:00")
    ...
    expect_error(quickcalls(df, confidence = 0.5), "one or fewer species detected")
})
